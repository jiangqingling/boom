...
$('.onadd').on('click',function () {
    ...
    $(this).siblings('.onreduce').removeClass('disabled');
    moneyChange();
});
// 减少按钮
$('.onreduce').on('click',function () {
    var val = parseInt($(this).siblings('input').val());
    if(val >1 && (val-1) !==1){
        $(this).siblings('input').val(val - 1);
    }
...
$('.t_count input').on('input',function () {
    ...
    if( 1 >= val){
        $(this).siblings('.onreduce').addClass('disabled')
    }
    else if(val > 1){
        $(this).siblings('.onreduce').removeClass('disabled')
    }
